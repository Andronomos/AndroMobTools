...
public class BlockMinerContainer extends BaseContainerMenu {
	public BlockMinerBE blockEntity;

	public BlockMinerContainer(int windowId, BlockPos pos, Inventory inventory) {
		super(ModContainers.BLOCK_MINER.get(), windowId, inventory);

		BlockEntity blockEntityAtPos = this.player.getCommandSenderWorld().getBlockEntity(pos);

		if(blockEntityAtPos != null && blockEntityAtPos instanceof BlockMinerBE blockMiner) {
			blockEntity = blockMiner;
	...
	public boolean stillValid(Player player) {
		return stillValid(ContainerLevelAccess.create(blockEntity.getLevel(), blockEntity.getBlockPos()), player, ModBlocks.BLOCK_MINER.get());
	}
}
